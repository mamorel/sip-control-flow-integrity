...
A simple way to defeat this kind of attack is applying an additional layer of obfuscation to this part of the program. This would make reverse engineering more complicated.% I'm not really sure if white-box cryptography would help here. The question is: how do we derive the key? wbc gets the key from the program. If the program (or the important parts for wbc) is not modified but only the text file is corrupted, what do we gain? But yes, HMAC instead of a simple checksum would be a good improvement

An attacker might also defeat our scheme by bypassing the calls to the shadow stack building functions. In the \texttt{readKey} example, an attacker can jump from any function to the \texttt{readKey} function provided that the destination is located after the call to the \texttt{registerFunction}. % I don't understand this point. The call of registerFunction is done at the beginning of the function? I think, this should be ok with the paragraph above?
...
A summary of the attacks and possible corresponding countermeasures is shown in \autoref{fig:adtree}.
\begin{figure}[htb]
